...
- `scripts/plot_today_trades.mjs` + `scripts/afterhours_plot.sh` — after-market job: runs the trade_plotter generator, then for each traded symbol **updates the saved `<SYM> Trades Plotter` Pine script in place** (open → set source → Ctrl+S). Chart studies bound to that saved script auto-refresh; no "Add to chart", no unsaved "My script", no duplicate `… Plotter 1` scripts. (v1 added the *unsaved* editor script, which this TV build silently added as the blank `plot(close)` "My script" template — so charts never updated — and the save dialog spawned duplicates.) **One-time per symbol:** the study must already be on a chart bound to the saved script (add via Indicators → My scripts); symbols with no saved script are skipped. Cron: `0 16 * * 1-5` (16:00 CDT). **Run log: `logs/afterhours_plot.log`** (gitignored).
