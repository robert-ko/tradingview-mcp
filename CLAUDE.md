...
- **Alert dashboard** (`node scripts/dashboard.js --port=8787`, local-only): grid UI to enable/disable/add alerts by symbol × type × tab, save state to `alerts_config.json`, and a "Recently fired" panel (polls each alert's `last_fire_time`). **Full guide: `docs/DASHBOARD.md`.** To receive webhook fires you must expose it through a tunnel — **TradingView fires from its cloud and rejects `localhost`, accepting only public http/https on ports 80/443**. Use `scripts/tunnel.sh` (ngrok) + set `WEBHOOK_URL`/`WEBHOOK_SECRET`, then "Webhook all". Full tunnel runbook incl. the SafeBrowse local-filter gotcha: `docs/WEBHOOK_TUNNEL.md`.
...
- `scripts/plot_today_trades.mjs` + `scripts/afterhours_plot.sh` — after-market job: runs the trade_plotter generator, then for each traded symbol **overwrites the saved `<SYM> Trades Plotter` Pine script in place via the pine-facade REST endpoint** — `POST /pine-facade/save/next/{scriptId}?allow_create_new=false&name=…` with FormData `{source}`, run inside a TV page for the session cookie. Renderer-independent, no editor, no duplicate `… Plotter 1` scripts. (v1 clicked "Add to chart" of the *unsaved* editor script → this TV build silently added the blank `plot(close)` "My script" template, so charts never updated; v2 used editor Ctrl+S, which is a **no-op** unless the editor is focused AND bound to that script — `monaco.setValue` doesn't mark the doc dirty — so it wrote nothing.) `--reload` optionally removes + re-adds the on-chart study (Indicators → My scripts) so panes show today's trades — existing studies keep the version they were added with. Symbols with no saved script are skipped (never auto-created). Cron: `0 16 * * 1-5` (16:00 CDT). **Run log: `logs/afterhours_plot.log`** (gitignored). **Full guide: `docs/TRADE_PLOTTER.md`.**
