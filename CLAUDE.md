...
- **Alert dashboard** (`node scripts/dashboard.js --port=8787`, local-only): grid UI to enable/disable/add alerts by symbol × type × tab, save state to `alerts_config.json`, and a "Recently fired" panel (polls each alert's `last_fire_time`). To receive webhook fires you must expose it through a tunnel — **TradingView fires from its cloud and rejects `localhost`, accepting only public http/https on ports 80/443**. Use `scripts/tunnel.sh` (ngrok) + set `WEBHOOK_URL`/`WEBHOOK_SECRET`, then "Webhook all". Full runbook incl. the SafeBrowse local-filter gotcha: `docs/WEBHOOK_TUNNEL.md`.
